...
/*   Updated: 2026/01/25 13:23:39 by adruz-to         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_wall	init_wall(t_game *game, t_ray *ray)
{
	...
	// TODO: revisar es segunda condición: ray->side == 1 && ray->dir_y < 0
...
uint32_t	get_tex_color(mlx_texture_t *texture, int x, int y)
{
	...
	return ((texture->pixels[index] << 24) | (texture->pixels[index + 1] << 16) 
